...
ll solve(ll i)
{
    ...
    ll ans1,ans2;
    // if(i<n-1)
     ans1=abs(arr[i]-arr[i-1])+solve(i-1);
    // else
    // {
    //     return 0;
    // }
    // if(i<n-2)
    if(i>1)
     ans2=abs(arr[i]-arr[i-2])+solve(i-2);
     else 
     ans2=abs(arr[i]-arr[i-1])+solve(i-1);
    // else
    // {
    //     return(min(abs(arr[n-2]-arr[n-3])+abs(arr[n-2]=arr[n-1]),abs(arr[n-3]-arr[n-1])));
    // }
    dp[i]=min(ans1,ans2);
    return dp[i];
}
